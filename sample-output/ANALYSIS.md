...
`trace.jsonl` 是默认配置下 C 场景的完整记录，735 个事件；`metrics.json` 汇总 A–E。`manifest.json` 保存生成配置，`completion-validation.json` 记录核心补齐阶段的工作区版本和产物哈希，`dev-validation.json` 记录随后开发入口与信号退出的验证；各 validation 文件保留为对应版本的历史记录。两次 release 运行的 A–E trace 均逐字节相同；`sequence.md` 可直接在支持 Mermaid 的 Markdown 阅读器中查看。
